...
# Prompt to enter a password for the PostgreSQL bistbin user (rather than hard-coding
# a password in this script).
read -p "Enter password for gistbin DB user: " DB_PASSWORD
...
# Set up the gistbin DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE gistbin"
sudo -i -u postgres psql -d gistbin -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d gistbin -c "CREATE ROLE gistbin WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the gistbin database to the system-wide environment 
# variables in the /etc/environment file.
echo "GISTBIN_DB_DSN='postgres://gistbin:${DB_PASSWORD}@localhost/gistbin'" >> /etc/environment
